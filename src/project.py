...
def main(num_epoch, load_mode=None, net_mode='all_path', model_name='ALLOT', seed=''):
    if len(seed) > 0:
        seed = int(seed)
        system_init(seed)
    else:
    ...
    run_manager.pt_project(cfg.trainer.project_init)
...
if __name__ == '__main__':
    ...
    model_name = cfg.model.name + '_' + args.desc
    if len(args.opt) > 0:
        model_name += '_' + args.opt
    main(args.epoch, load_mode=args.load_mode, net_mode=args.net_mode, model_name=model_name, seed=args.seed)
