...
def main(num_epoch, load_mode='final_project', net_mode='all_path', model_name='ALLOT', seed='', opt=''):
    if len(seed) > 0:
        seed = int(seed)
        system_init(seed)
    else:
    ...
    run_manager._load(exp_mode=load_mode)
    ...
    run_manager.train(num_epoch, net_mode)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_mode', type=str, default='final_project') # search or final_project
    parser.add_argument('--net_mode', type=str, default='one_fixed')
    ...
    main(args.epoch, load_mode=args.load_mode, net_mode=args.net_mode, model_name=model_name, seed=args.seed, opt=args.opt)
