...
class Architecture(nn.Module):
    ...
    def next_cell(self, cell_id):
        for idx in range(cell_id, len(self._model._layers)):
            if isinstance(self._model._layers[idx], self._model._cell_type):
                remain_eids = torch.nonzero(self._model._layers[idx]._candidate_flags).cpu().numpy().T[0]
    ...
    def remain_edge_ids(self, cell_id):
        return torch.nonzero(self._model._layers[cell_id]._candidate_flags).cpu().numpy().T[0]
    ...
    def project_masked_weights(self, cell_id, e_id, op_id):
        ...
        proj_mask = torch.ones_like(weights[e_id])
        proj_mask[op_id] = 0
    ...
    def clip(self):
        for p in self._model.arch_parameters():
            for line in p:
