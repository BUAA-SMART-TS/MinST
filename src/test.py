...
def main(num_epoch, load_mode=None, net_mode='all_path', model_name='ALLOT', seed='', opt=''):
    if len(seed) > 0:
        seed = int(seed)
        system_init(seed)
    else:
    ...
    run_manager._load(exp_mode='train')
    ...
    run_manager.test(net_mode=net_mode)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_mode', type=str, default='train') # search or final_project
    ...
    main(args.epoch, net_mode=args.net_mode, model_name=model_name, seed=args.seed, opt=args.opt)
